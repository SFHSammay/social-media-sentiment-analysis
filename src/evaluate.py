...
from src.config import TOP_K, LABELS

def precision_at_k(retrieved_doc_ids: list[int], relevant_doc_ids: set[int], k: int = TOP_K) -> float:
    top_k = retrieved_doc_ids[:k]
    hits  = sum(1 for d in top_k if d in relevant_doc_ids)
    return hits / k if k > 0 else 0.0

def average_precision(retrieved_doc_ids: list[int], relevant_doc_ids: set[int]) -> float:
    hits, score = 0, 0.0
...
def mean_average_precision(results_per_query: dict[str, list[int]],
                            relevant_per_query: dict[str, set[int]]) -> float:
    aps = []
...
def evaluate_ir(index: InvertedIndex, search_fn, queries: list[str]) -> dict:
    ...
    return {"MAP": round(map_score, 4), "per_query": per_query, "retrieved": retrieved_per_query}
