...
from src.config import BM25_B, BM25_K1, QL_JM_LAMBDA, QL_MU
from src.retrieval.indexer import InvertedIndex


class TFIDFScorer:
    # Scores docs against a query using TF-IDF
    ...
    def rank(self, query_tokens: list[str], top_k: int = 10) -> list[tuple[int, float]]:
        # Return top_k pairs (doc_id,score)
        candidates: dict[int, float] = {}
        for term in query_tokens:
            for doc_id in self.index.index.get(term, {}):
                candidates[doc_id] = candidates.get(doc_id, 0.0) + self.score(
                    [term], doc_id
                )
...
class BM25Scorer:
    # Scores docs against a query using BM25
    def __init__(self, index: InvertedIndex):
        self.index = index
        self.k1 = BM25_K1
        self.b = BM25_B
    ...
    def rank(self, query_tokens: list[str], top_k: int = 10) -> list[tuple[int, float]]:
        ...
        for term in query_tokens:
            ...
            for doc_id, tf in term_postings.items():
                ...
                candidates[doc_id] = candidates.get(doc_id, 0.0) + idf * (
...
class QLDPScorer:
    # Scores docs against a query using Query Likelihood with Dirichlet smootihign
    def __init__(self, index: InvertedIndex):
        self.index = index
        self.mu = QL_MU
...
class QLJMScorer:
    # Just like QLDPScorer, but with JM smoothing instead
    def __init__(self, index: InvertedIndex):
        self.index = index
